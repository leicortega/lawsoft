...
$(document).ready(function() {
    ...
    var options = {
        ...
        series: [
            {
                name: 'Procesos',
                data: generateDayWiseTimeSeries((new Date().getTime() - diezDias), 11, {
                    min: 0,
                    max: 15
                })
            }
    ...
    function generateDayWiseTimeSeries(baseval, count, yrange) {
        ...
        var fecha = new Date(baseval).toLocaleDateString();

        $.ajax({
            url: '/get_procesos_for_day',
            type: 'POST',
            data: { fecha:fecha },
...
$(document).ready(function() {
    ...
    var options = {
        ...
        labels: ['Civil', 'Familia', 'Laboral', 'Seguridad Social', 'Administrativo', 'Penal', 'Otros'],
        colors: ['#551bd2', '#6435c9', '#7547d6', '#a07af1', '#9773e4', '#a28ad6', '#b19fda'],
   ...
   var chart = new ApexCharts(
    ...
    function cargar_procesos() {
        $.ajax({
            url: '/get_procesos_for_type',
            type: 'POST',
            success: (data) => {
                for (let index = 0; index < data.length; index++) {
                    data_series.push(data[index]);
                }

                return chart.updateSeries(data_series);
            }
        });
    }
});
...
$(document).ready(function() {
    ...
    var options = {
        ...
        labels: ['Clientes', 'Demandados'],
        colors: ['#551bd2', '#a07af1'],
        series: [0, 0],
        responsive: [{
            breakpoint: 480,
            options: {
   ...
   var chart = new ApexCharts(
        document.querySelector("#apex-simple-pie"),
        options
    );
